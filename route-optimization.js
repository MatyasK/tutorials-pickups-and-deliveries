...
const api_key =
  "RVVfYWI1YTZjOTQ3M2EyNGM4Zjg2MTJlMTcwMTRlZjllN2Q6YWNmYzRhZWMtNzNlZC00NmNhLWE0M2MtZGFjZDlmOTJjZTFh";
const APIEndpoints = {
  ...
  StartOptimization: (planId) =>
    `https://api.myptv.com/routeoptimization/v1/plans/${planId}/operation/optimization?tweaksToObjective=IGNORE_MINIMIZATION_OF_NUMBER_OF_ROUTES`,
  GetOptimizationProgress: (planId) =>
    `https://api.myptv.com/routeoptimization/v1/plans/${planId}/operation`,
  GetOptimizedPlan: (planId) =>
    `https://api.myptv.com/routeoptimization/v1/plans/${planId}`,
  GetImageTiles:
    "https://api.myptv.com/rastermaps/v1/image-tiles/{z}/{x}/{y}?size={tileSize}&style=silica",
};
...
const initializeApplication = () => {
  ...
  getElement("pickup-location").addEventListener(
    "input",
    debounce((event) => findLocationsByText(event, ServiceTypes.Pickup), 250)
  );
  getElement("delivery-location").addEventListener(
    "input",
    debounce((event) => findLocationsByText(event, ServiceTypes.Delivery), 250)
  );
  getElement("btn-add-transport").addEventListener("click", addTransport);
  getElement("btn-start-optimization").addEventListener(
    "click",
    optimizeTransports
  );
  getElement("previous-vehicle").addEventListener("click", () =>
    switchSelectedVehicle(-1)
  );
  getElement("next-vehicle").addEventListener("click", () =>
    switchSelectedVehicle(1)
  );
  getElement("close-error-details").addEventListener("click", () =>
    hideElement("error-log")
  );
  getElement("clear-transports").addEventListener("click", () =>
    clearSpecifiedTransports()
  );
...
const createOptimizationPlan = () => {
  ...
  const vehicleProfile =
    vehicleProfileSelection.options[vehicleProfileSelection.selectedIndex]
      .value;
...
const getOptimizationProgress = (planId) =>
  fetch(APIEndpoints.GetOptimizationProgress(planId), applyHeaders()).then(
    (response) => (response.ok ? response.json() : logError(response))
  );

const getOptimizedPlan = (planId) =>
  fetch(APIEndpoints.GetOptimizedPlan(planId), applyHeaders()).then(
    (response) => (response.ok ? response.json() : logError(response))
  );
...
const findLocationsByText = (inputEvent, serviceType) => {
  ...
  fetch(APIEndpoints.SearchLocations(value), applyHeaders())
    ...
    .then(({ locations }) => {
      ...
      locations.forEach((location) => {
        ...
        suggestion.addEventListener("click", () =>
          selectLocationSuggestion(location, id, serviceType)
        );
...
const selectLocationSuggestion = (location, inputElementId, serviceType) => {
  getElement(
    inputElementId
  ).value = `${location.formattedAddress}, ${location.address.countryName}`;
  ...
  if (appState.actions.pickup && appState.actions.delivery)
    enableElement("btn-add-transport");
};

const getOpeningTimes = (isPickup) => {
  const openingFromValue = isPickup
    ? getElement("pickup-from").value
    : getElement("delivery-from").value;
  const openingToValue = isPickup
    ? getElement("pickup-to").value
    : getElement("delivery-to").value;

  const [hoursFrom, minutesFrom] = openingFromValue
    .split(":")
    .map((value) => Number(value));
  const [hoursTo, minutesTo] = openingToValue
    .split(":")
    .map((value) => Number(value));
...
const createNewTransport = (pickupLocationId, deliveryLocationId) => {
  ...
  const deliveryServiceTimeSeconds =
    getElement("delivery-service-time").value * 60;
...
const addTransport = () => {
  ...
  appState.actions.pickupAndDeliveries.push(
    { id: pickupLocation.id, ...pickup },
    { id: deliveryLocation.id, ...delivery }
  );
  appState.locations.push(pickupLocation);
  appState.locations.push(deliveryLocation);

  const newTransport = createNewTransport(
    pickupLocation.id,
    deliveryLocation.id
  );
...
const clearSpecifiedTransports = () => {
  ...
  removeAllChildNodes(
    getElement("transports-overview").getElementsByTagName("tbody")[0]
  );
  disableElement("btn-start-optimization");
};

const updateTransportsOverviewTable = (transport) => {
  const tbody = getElement("transports-overview").getElementsByTagName(
    "tbody"
  )[0];
  const row = tbody.insertRow();
  row.insertCell(0).innerText = transport.id;
  row.insertCell(1).innerText = lookupLocation(
    transport.pickupLocationId
  ).formattedAddress;
  row.insertCell(2).innerText = lookupLocation(
    transport.deliveryLocationId
  ).formattedAddress;
};
...
const switchSelectedVehicle = (step) => {
  ...
  const usedVehicles = vehicles.filter(
    (vehicle) => !unplannedVehicleIds.includes(vehicle.id)
  );
  ...
  route.stops.forEach((stop, index) => {
    ...
    eventCell.innerText =
      stop.deliveryIds.length > 0
        ? "Delivery"
        : stop.pickupIds.length > 0
        ? "Pickup"
        : "Vehicle Location";
    arrivalTimeCell.innerText = new Date(
      stop.reportForStop.arrivalTime
    ).toLocaleTimeString();
  });

  getElement("vehicle-travel-distance").innerText = formatMetersToKilometers(
    route.report.distance
  );
  getElement("vehicle-travel-time").innerText = formatSecondsToHHMM(
    route.report.travelTime
  );
};
...
const populateKPIs = () => {
  const {
    ...
    vehicles,
    unplannedVehicleIds,
  } = appState.optimizedPlan;
  const totalTravelTime = routes.reduce(
    (sum, route) => sum + route.report.travelTime,
    0
  );
  const totalDrivingTime = routes.reduce(
    (sum, route) => sum + route.report.drivingTime,
    0
  );
  const totalDistance = routes.reduce(
    (sum, route) => sum + route.report.distance,
    0
  );
  const totalBreakTime = routes.reduce(
    (sum, route) => sum + route.report.breakTime,
    0
  );
  const totalRestTime = routes.reduce(
    (sum, route) => sum + route.report.restTime,
    0
  );
  const totalWaitingTime = routes.reduce(
    (sum, route) => sum + route.report.waitingTime,
    0
  );

  getElement("used-vehicles").innerText =
    vehicles.length - unplannedVehicleIds.length;
  getElement("unused-vehicles").innerText = unplannedVehicleIds.length;
  getElement("planned-transports").innerText =
    transports.length - unplannedTransportIds.length;
  getElement("unplanned-transports").innerText = unplannedTransportIds.length;
  getElement("total-travel-time").innerText =
    formatSecondsToHHMM(totalTravelTime);
  getElement("total-driving-time").innerText =
    formatSecondsToHHMM(totalDrivingTime);
  getElement("total-travel-distance").innerText =
    formatMetersToKilometers(totalDistance);
  getElement("total-break-time").innerText =
    formatSecondsToHHMM(totalBreakTime);
  getElement("total-rest-time").innerText = formatSecondsToHHMM(totalRestTime);
  getElement("total-waiting-time").innerText =
    formatSecondsToHHMM(totalWaitingTime);
};
